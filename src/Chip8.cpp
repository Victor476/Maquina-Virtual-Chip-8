#include "Chip8.h"
#include <cstring>  // Para std::memset e std::memcpy
#include <iomanip>  // Para formatação de saída (Validação)
#include "components/TimerManager.h"
#include "components/Display.h" 

// Dados dos sprites dos dígitos hexadecimais (0-F). 
// 16 sprites * 5 bytes/sprite = 80 bytes totais.
const uint8_t CHIP8_FONTSET[80] = {
    0xF0, 0x90, 0x90, 0x90, 0xF0, // 0
    0x20, 0x60, 0x20, 0x20, 0x70, // 1
    0xF0, 0x10, 0xF0, 0x80, 0xF0, // 2
    0xF0, 0x10, 0xF0, 0x10, 0xF0, // 3
    0x90, 0x90, 0xF0, 0x10, 0x10, // 4
    0xF0, 0x80, 0xF0, 0x10, 0xF0, // 5
    0xF0, 0x80, 0xF0, 0x90, 0xF0, // 6
    0xF0, 0x10, 0x20, 0x40, 0x40, // 7
    0xF0, 0x90, 0xF0, 0x90, 0xF0, // 8
    0xF0, 0x90, 0xF0, 0x10, 0xF0, // 9
    0xF0, 0x90, 0xF0, 0x90, 0x90, // A
    0xE0, 0x90, 0xE0, 0x90, 0xE0, // B
    0xF0, 0x80, 0x80, 0x80, 0xF0, // C
    0xE0, 0x90, 0x90, 0x90, 0xE0, // D
    0xF0, 0x80, 0xF0, 0x80, 0xF0, // E
    0xF0, 0x80, 0xF0, 0x80, 0x80  // F
};

Chip8::Chip8() {
    // Construtor chama a inicialização padrão
    initialize();
}

void Chip8::initialize() {
    // 1. Zera todos os componentes
    std::memset(memory.data(), 0, memory.size()); 
    std::memset(V, 0, sizeof(V));               
    std::memset(stack, 0, sizeof(stack));       

    // Zera registradores e timers
    I = 0;
    SP = 0;
    delay_timer = 0;
    sound_timer = 0;
    opcode = 0;

    // 2. Configura o Program Counter (PC)
    // Critério: PC inicializado em 0x200
    PC = 0x200; 

    // 3. Pré-carrega o Font Set na Memória
    // Critério: Carregar sprites na memória, começando em 0x000.
    std::memcpy(memory.data(), CHIP8_FONTSET, sizeof(CHIP8_FONTSET));

    // --- Validação (Teste Unitário/Print) ---
    std::cout << "--- Chip-8 VM Inicializada ---" << std::endl;
    std::cout << "PC: 0x" << std::hex << std::setw(4) << std::setfill('0') << PC << " (Esperado: 0x0200)" << std::endl;
    std::cout << "I: 0x" << (int)I << ", SP: " << (int)SP << ", DT: " << (int)delay_timer << ", ST: " << (int)sound_timer << " (Esperado: 0)" << std::endl;
    
    // Validação da Pré-carga (Inspeção)
    std::cout << "Memória[0x000]: 0x" << std::hex << (int)memory[0x000] << " (Esperado: 0xF0)" << std::endl;
    std::cout << "Memória[0x050]: 0x" << std::hex << (int)memory[0x050] << " (Esperado: 0x00)" << std::endl;
}

// Implementações mínimas para evitar erros de linkagem
void Chip8::load_rom(const char* filename, uint16_t load_address) { /* Implementação na próxima Issue */ }
void Chip8::cycle() { /* Loop principal aqui */ }
void Chip8::load_font_set() { /* Não utilizado, pois foi integrado em initialize() */ }